            ...
            <div class="contract_info">
                ...
                <div class="details">
                    <div class="static">
                        ...
                        <div>
                            <label>Prijs van tickets (&euro;):</label><span
                                    class="price">{{ number_format((float)$contract->advertisements->price, 2, '.', '') }}</span>
                        </div>
                        <div>
                            <label>Prijs van Que'r (&euro;):</label><span
                                    class="price">{{ number_format((float)$contract->price, 2, '.', '') }}</span>
                        </div>
                        <div>
                            <label>Totale prijs (&euro;):</label><span
                                    class="price">{{ number_format((float)($contract->advertisements->price + $contract->price), 2, '.', '') }}</span>
                        </div>
                    ...
                    <div class="variable">
                        @if(Auth::user()->id == $quer->id && $contract->phases->phase_number == 10)
                            <div>
                                <div>
                                    Als je het ticket gekocht hebt, upload het dan hier zodat de aanvrager het kan
                                    downloaden. Het geld wordt overgemaakt zodra de aanvrager het ticket geaccepteerd
                                    heeft.
                                </div>
                                <form id="upload_ticket" action="{{ url('upload_ticket') }}" method="POST"
                        ...
                        @if(Auth::user()->id == $quer->id && $contract->phases->phase_number != 10 && $contract->phases->phase_number < 25)
                            <div>
                                Bedankt voor het uploaden van het ticket! Van zodra de apply'r het ticket geaccepteerd
                                heeft, wordt je geld overgemaakt!
                            </div>
                        @endif
                        ...
                        @if(Auth::user()->id == $applicant->id && $contract->phases->phase_number >= 15)
                            <div>
                                ...
                                @if($contract->phases->phase_number < 25)
                                    <form id="accept_ticket" action="{{ url('accept_ticket') }}" method="POST"
                                          enctype="multipart/form-data">
                                        {{ csrf_field() }}
                        ...
                        @if(Auth::user()->id == $applicant->id && $contract->phases->phase_number >= 20 && $contract->phases->phase_number != 30)
                            <div class="review">
                                <form id="add_review" action="{{ url('add_review') }}" method="POST">
                                    ...
                                    <input type="hidden" name="advertisement_id" id="add_rev_advertid"
                                           value="{{$contract->advertisement_id}}">
                                    <input type="hidden" name="contract_id" value="{{$contract->id}}">
                                    ...
                                    <div class="rate">
                                        ...
                                        <div><input type="radio" name="rate" value="1" id="1"><label for="1">1</label>
                                        </div>
                                        <div><input type="radio" name="rate" value="2" id="2"><label for="2">2</label>
                                        </div>
                                        <div><input type="radio" name="rate" value="3" id="3"><label for="3">3</label>
                                        </div>
                                        <div><input type="radio" name="rate" value="4" id="4"><label for="4">4</label>
                                        </div>
                                        <div><input type="radio" name="rate" value="5" id="5"><label for="5">5</label>
                                        </div>
                                    </div>
